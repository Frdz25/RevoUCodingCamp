...
- [ ] 2. Implement `renderSpecies()` — inject species cards into `#species-grid`
  ...
  - [-] 2.2 Write property test for species card rendering completeness
  ...
  - [~] 2.3 Write property test for image alt attributes
...
- [ ] 3. Implement `renderStats()` — inject stat counters into `#stats-grid`
  - [~] 3.1 Write `renderStats()` to map the `stats` array to HTML strings and set `#stats-grid.innerHTML`
    ...
    - _Requirements: 6.1_
  - [~] 3.2 Write property test for stats rendering count
...
- [ ] 4. Implement `animateCounters()` — scroll-triggered counter animation
  - [~] 4.1 Write `animateCounters()` using `IntersectionObserver` (threshold 0.5) to animate each `.stat-number` from 0 to its `data-target` value over ~2000ms using `setInterval`
  ...
  - [~] 4.2 Add `IntersectionObserver` fallback: if `typeof IntersectionObserver === 'undefined'`, set each `.stat-number` `textContent` to `target.toLocaleString() + suffix` immediately
    - _Requirements: 6.4_
  - [~] 4.3 Write property test for counter animation reaching target value
  ...
  - [~] 4.4 Write property test for counter animation running only once
...
- [ ] 5. Implement `renderDonationAmounts()` — inject preset donation buttons
  - [~] 5.1 Write `renderDonationAmounts()` to inject `.amount-btn` buttons into `#donation-amounts`, each with a `data-value` attribute matching the source data
    - _Requirements: 7.1_
  - [~] 5.2 Attach click listeners to each `.amount-btn`: on click, remove `.active` from all buttons, add `.active` to the clicked button, and set `#custom-amount.value` to the button's `data-value`
    - _Requirements: 7.2, 7.3_
  - [~] 5.3 Write property test for donation buttons rendering count
  ...
  - [~] 5.4 Write property test for donation button click populating amount input
  ...
  - [~] 5.5 Write property test for donation button active state mutual exclusion
...
- [~] 6. Checkpoint — Ensure rendering and data tasks pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Implement `initNavbar()` — scroll-triggered navbar style
  - [~] 7.1 Write `initNavbar()` to attach a `scroll` event listener on `window`; toggle class `.scrolled` on `#navbar` when `window.scrollY > 60`
    - _Requirements: 1.2, 1.3, 1.4_
  - [~] 7.2 Write unit tests for navbar scroll behavior
    - Simulate scroll events at 0px, 60px, and 61px; verify `.scrolled` is absent at ≤60px and present at >60px
    - _Requirements: 1.3, 1.4_

- [ ] 8. Implement `initMobileMenu()` — full-screen overlay navigation
  - [~] 8.1 Write `initMobileMenu()` to attach click listeners: `#menu-btn` removes `hidden` from `#mobile-menu`; `#close-menu` and each `.mobile-link` add `hidden` back
    - _Requirements: 2.2, 2.3, 2.4_
  - [~] 8.2 Write unit tests for mobile menu open/close
...
- [ ] 9. Implement `initScrollObserver()` — fade-in on scroll
  - [~] 9.1 Write `initScrollObserver()` using `IntersectionObserver` (threshold 0.15) to add class `.visible` to each `.fade-in` element when it enters the viewport, then call `observer.unobserve()` on it (one-shot)
    - _Requirements: 5.4 (implicit from design), 6.1_

- [ ] 10. Implement `initForms()` — form submission handlers
  - [~] 10.1 Write `initForms()` to attach a `submit` listener on `#donate-form`: call `e.preventDefault()`, show `#form-msg` (remove `hidden`), reset the form, hide `#form-msg` after 4 seconds
    - _Requirements: 7.4, 7.5, 7.6_
  - [~] 10.2 Attach a `submit` listener on `#contact-form` with the same pattern: show `#contact-msg`, reset, hide after 4 seconds
    - _Requirements: 8.3, 8.4_
  - [~] 10.3 Write unit tests for form submission (valid)
    - Fill all required fields, submit, assert success message is visible and form is reset
    - _Requirements: 7.5, 8.3_
  - [~] 10.4 Write unit tests for form submission (invalid)
    - Submit empty form, assert `form.checkValidity()` returns false and submission is prevented
    - _Requirements: 7.6, 8.4_

- [ ] 11. Wire all init functions in `DOMContentLoaded`
  - [~] 11.1 Ensure `DOMContentLoaded` calls `renderSpecies()`, `renderStats()`, `renderDonationAmounts()`, `initNavbar()`, `initMobileMenu()`, and `initForms()` in order
  - [~] 11.2 Call `initScrollObserver()` and `animateCounters()` inside a `setTimeout(..., 100)` to ensure rendered elements are in the DOM before observers attach
    - _Requirements: 5.6, 6.3_

- [ ] 12. Verify `css/style.css` custom styles are complete
  - [~] 12.1 Confirm `html { scroll-behavior: smooth; }` is present for smooth anchor scrolling
    - _Requirements: 1.5, 1.6, 3.5, 3.6_
  - [~] 12.2 Confirm `#navbar` transparent default and `#navbar.scrolled` solid background + shadow styles are present
    - _Requirements: 1.3, 1.4_
  - [~] 12.3 Confirm `.species-card` hover transform and `.fade-in` / `.fade-in.visible` transition styles are present
    - _Requirements: 5.2_
  - [~] 12.4 Confirm `.amount-btn.active` active state styles are present
    - _Requirements: 7.3_

- [ ] 13. Verify `index.html` static structure and accessibility
  - [~] 13.1 Confirm all required section IDs exist in order: `#navbar`, `#hero`, `#about`, `#species`, `#impact`, `#donate`, `#contact`, footer
    - _Requirements: 1.1_
  - [~] 13.2 Confirm `<meta name="viewport">` tag is present
    - _Requirements: 10.4_
  - [~] 13.3 Confirm Tailwind CDN `<script>` tag is present in `<head>` and `js/main.js` `<script>` tag is before `</body>`
    - _Requirements: 10.1, 10.2_
  - [~] 13.4 Confirm all static `<img>` elements have non-empty `alt` attributes
    - _Requirements: 10.5_
  - [~] 13.5 Confirm footer contains at least 4 social media links (Instagram, Twitter, Facebook, YouTube)
    - _Requirements: 9.2_
  - [~] 13.6 Confirm responsive Tailwind grid classes are present on containers (`md:grid-cols-2`, `md:grid-cols-3`)
